...
class CodeEncryptor:
    ...
    def _get_machine_id(self) -> str:
        ...
        fallback_file = Path.home() / ".alonechat" / ".machine_id"
        if fallback_file.exists():
            try:
                stored = fallback_file.read_text(encoding="utf-8").strip()
                if stored:
                    return stored
            except Exception:
                pass
        ...
        fallback_file.write_text(random_id, encoding="utf-8")
        try:
