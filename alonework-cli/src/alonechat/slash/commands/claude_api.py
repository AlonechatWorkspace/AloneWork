...
def _get_encryption_key() -> bytes:
    ...
    if salt_file.exists():
        try:
            salt = base64.b64decode(salt_file.read_text(encoding="utf-8").strip())
        except Exception:
            salt = os.urandom(32)
    else:
        salt = os.urandom(32)
        ...
        if config_file.exists():
            try:
                with open(config_file, "r", encoding="utf-8") as f:
                    data = json.load(f)
                if data.get("api_key") and data.get("_api_key_encrypted"):
                    try:
    ...
    def _save_config(config: dict) -> None:
        ...
        if save_data.get("api_key"):
            try:
                ...
                save_data["_api_key_encrypted"] = True
            except Exception:
                pass
