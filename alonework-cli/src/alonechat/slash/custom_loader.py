...
BASH_WHITELIST = {
    ...
    "date", "whoami", "which", "type", "sort", "uniq", "git", "pip",
    "npm", "node", "python", "python3", "cargo", "go", "make",
}
...
def process_bash_commands(prompt: str) -> str:
    ...
    安全增强: 禁止 shell=True，只允许白名单命令 / Security: no shell=True, whitelist-only
    """
    pattern = r'!`([^`]+)`'
    ...
    def replace_bash(match):
        ...
        try:
            ...
            return f"[Error: Invalid command syntax: {command}]"

        if not args:
            return "[Error: Empty command]"

        cmd_name = args[0]
        if cmd_name not in BASH_WHITELIST:
            return f"[Error: Command '{cmd_name}' not in whitelist]"

        shell_metacharacters = set(';&|`$(){}[]<>#!')
        if any(c in command for c in shell_metacharacters):
            return f"[Error: Shell metacharacters not allowed in: {command}]"
...
def _is_safe_path(file_path: Path) -> bool:
    """
    校验路径安全性，防止路径遍历 / Validate path safety to prevent path traversal
    """
    sensitive_dirs = [
        Path("/etc"), Path("/proc"), Path("/sys"), Path("/dev"),
        Path("/var"), Path("/usr"), Path("/bin"), Path("/sbin"),
...
def process_file_references(prompt: str) -> str:
    ...
    def replace_file(match):
        ...
        if not _is_safe_path(file_path):
            return f"[Security: Path not allowed: {raw_path}]"
